...
## Phase 6 — Persistence, auth, public deployment · completed 2026-07-29

**Live:** frontend [urbanheat-mumbai.vercel.app](https://urbanheat-mumbai.vercel.app) ·
backend [urbanheat-api.onrender.com](https://urbanheat-api.onrender.com)

### Added
- `supabase/schema.sql` — `saved_scenarios` table (config only: `ward_code`, `intervention`,
  `coverage`, `user_id`, `saved_at` — loading one re-runs the real `/scenario` call, never a
  stored snapshot) with row-level security; the one place this project holds per-user data
- `backend/auth.py` — `GET /auth/me` and every write endpoint verify a Supabase session by
  asking Supabase's own `/auth/v1/user`, not decoding the JWT locally; magic-link sign-in in
  `frontend/src/auth/`
- `backend/saved_scenarios.py` + `GET`/`POST`/`DELETE /scenarios` — every call forwards the
  caller's own Supabase token to PostgREST, so RLS (not backend code) is what actually stops
  one user from touching another's rows; live-verified with two throwaway admin-created test
  users, including that a cross-user delete 404s rather than leaking whether the row exists
- `Dockerfile`, `render.yaml`, `.dockerignore`, `.github/workflows/ci.yml` — the backend ships
  as a pre-built image pushed to GHCR (Render never builds from source: the gitignored
  data/model/RAG artifacts it needs only exist on the machine that ran the pipeline, ADR-0004)
- `pyproject.toml` split into base (backend runtime) dependencies and a `pipeline` extra
  (Earth Engine, OSM, training/notebook tooling) — the deployed image installs the base set
  only
- ADR-0013 — RAG embeddings moved from a local `sentence-transformers`/`torch` model to
  Gemini's embedding API, after a live OOM kill on the first real deploy attempt traced the
  crash to `torch`'s ~500MB runtime footprint against Render free tier's 512MB ceiling

### Verified
- ✅ **Exit criterion met** — public URLs work end to end, author-confirmed 2026-07-29.
- The Docker image was smoke-tested under a **hard `--memory=512m` limit**, matching Render
  exactly, before ever being pushed — real container, real Supabase project, real endpoints,
  the RAG-backed agent supervisor initializing, settled at 339MB. It still hit a real OOM on
  the actual first Render deploy (backend/routers/agent.py's module-level import pulls the
  full agent stack in before uvicorn even binds a port — a difference the local
  `docker run` reproduction didn't fully capture until the memory limit was added), fixed via
  ADR-0013, and the second deploy attempt booted clean in production.
- Auth and saved-scenarios RLS were both live-verified against the real Supabase project with
  throwaway admin-created test users (no real email sent) before any UI work — cross-user
  isolation confirmed at the database level, not assumed from the policy text.
- Three more real bugs surfaced only by actually deploying, not by local testing: a trailing
  slash in `VITE_API_BASE_URL` silently produced double-slash request URLs; GHCR packages are
  private by default, 404ing an anonymous Render pull; a trailing slash in `CORS_ORIGINS`
  meant the configured origin never literally matched the `Origin` header a browser actually
  sends, which Starlette's CORS middleware rejects with a bare 400 and no diagnostic header.

### Known limitations carried into Phase 7
- **RAG embedding rate limit unconfirmed** (ADR-0013) — whether `gemini-embedding-001` shares
  the scarce 20 req/day chat quota (ADR-0011) or has its own separate limit isn't known;
  Google doesn't publish free-tier per-model numbers. Real usage will reveal it.
- Free-tier realities remain: Render sleeps after 15 min idle (~1 min cold start), Supabase
  pauses after ~1 week of inactivity — both require waking before a demo (`runbook.md` §5)
- No CI-driven deploy — pushing a new image and redeploying is a manual, author-run sequence
  (`runbook.md` §4), by design: the gitignored data/model artifacts it bakes in can't be
  regenerated by a CI runner without Earth Engine credentials
